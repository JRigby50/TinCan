...
'Description: Retrieves print server information and then writes the returned data to an Excel spreadsheet.
...
strComputer = InputBox ("Please type the print server name to check, " & vbCrLf & _
   "Else enter ALL for all print servers", "Server Name","RESVA-PRN01")

if strComputer = "" then
  WScript.quit
end if

strExcelPath = InputBox ("Please enter the path to save file to: ", "File path", "C:\AdminReports\")
...
Select Case UCase(strComputer)
  Case "ALL"
    PrintServer("RESVA-PRN01")
    Sheet = Sheet + 1
    PrintServer("RSMVAD-PRN01")
    Sheet = Sheet + 1
    PrintServer("RSMMDK-PRN01")
  Case Else
